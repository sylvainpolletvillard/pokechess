...
export function updateAlterations(pokemon: PokemonOnBoard, game: GameScene) {
	pokemon.alterations.forEach((alt) => {
		...
		if (alt.stacks <= 0) {
			...
			if (pokemonSprite) {
				alt.effectSprite.setPosition(
					pokemonSprite.x,
					pokemonSprite.y + (alt.effectDelta ?? 0),
				);
			} else {
...
export function applyAlterationEffect(
	...
	switch (alteration.type) {
		case AlterationType.POISON: {
			...
			//console.log(`Poison damage to ${pokemon.entry.ref}: ${poisonDamage}`);
		...
		case AlterationType.BRULURE: {
			...
			//console.log(`Burn damage to ${pokemon.entry.ref}: ${burnDamage}`);
			const gel = pokemon.alterations.find(
				(alt) => alt.type === AlterationType.GEL,
			);
			if (gel) gel.stacks -= 3;
			break;
		}
		...
		case AlterationType.LIGOTAGE: {
			const damage = pokemon.maxPV * (2 / 100) * perSecond; // 2% HP max per second
		...
		case AlterationType.TOURBILLON:
			switch (pokemon.facingDirection) {
				case Direction.UP:
					pokemon.facingDirection = Direction.RIGHT;
					break;
				case Direction.RIGHT:
					pokemon.facingDirection = Direction.DOWN;
					break;
				case Direction.DOWN:
					pokemon.facingDirection = Direction.LEFT;
					break;
				case Direction.LEFT:
				default:
					pokemon.facingDirection = Direction.UP;
					break;
			}
...
export function hasBlockingAlteration(pokemon: PokemonOnBoard) {
	return (
		pokemon.hasAlteration(AlterationType.TOURBILLON) ||
		pokemon.hasAlteration(AlterationType.SOMMEIL)
	);
}

export function addAlteration(
	pokemon: PokemonOnBoard,
	alteration: Alteration,
	game: GameScene,
) {
	...
	if (pokemon.item === BAIE_CERIZ) {
		...
		setTimeout(() => {
			pokemon.unalterable = false;
		}, 10 * 1000);
		pokemon.item = null;
		playSound("heal_ailment");
	}

	const alterationToStack = pokemon.alterations.find(
		(alt) => alt.type === alteration.type,
	);
	if (alterationToStack) {
		...
		/*console.log(
			`More stacks of ${alteration.type} on ${pokemon.entry.ref} (stacks: ${alterationToStack.stacks})`,
		);*/
		...
		if (alteration.type === AlterationType.RAGE) {
			...
			if (!targetSprite)
				return console.error(
					`Error, can't find pokemon sprite uid ${pokemon.uid}`,
				);
			targetSprite.setTint(
				Phaser.Display.Color.GetColor(
					255,
					clamp(200 - alteration.stacks * 40, 0, 255),
					clamp(128 - alteration.stacks * 30, 0, 255),
				),
			);
		}
	} else {
		/*console.log(
			`Apply ${alteration.type} on ${pokemon.entry.ref} (stacks: ${alteration.stacks})`,
		);*/
		const targetSprite = game.sprites.get(pokemon.uid);
		if (!targetSprite)
			return console.error(
		...
		switch (alteration.type) {
			case AlterationType.TOURBILLON: {
				pokemon.resetAction();
				const duration = alteration.stacks * game.gameSpeed;
			...
			case AlterationType.BRULURE:
				...
				alteration.effectSprite = makeEffectSprite(
					EFFECTS.BURN,
					targetSprite.x,
					targetSprite.y,
					game,
...
export function removeAlteration(pokemon: PokemonOnBoard, alt: Alteration) {
	...
	if (alt.type === AlterationType.SOMMEIL) {
		...
		const sprite = game.sprites.get(pokemon.uid);
		sprite?.anims.resume();
		const repos = pokemon.alterations.find(
			(alt) => alt.type === AlterationType.REPOS,
		);
		if (repos) removeAlteration(pokemon, repos);
	}

	if (
		alt.type === AlterationType.CONFUSION ||
		alt.type === AlterationType.GEL
	) {
		pokemon.resetAction();
	}
}
