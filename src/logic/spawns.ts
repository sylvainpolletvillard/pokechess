...
export function spawnTeamByTypeFactor(typesFactors: {
	[typeRef: string]: number;
}): PokemonOnBoard[] {
	...
	for (let i = 0; i < numberToSpawn; i++) {
		...
		team.push(
			new PokemonOnBoard({
				entry: ponderatedRandomIn(
					getPokemonsOfType(POKEMON_TYPES[type]),
					(p) => p.wildEncounterChance,
				),
				owner: NO_OWNER,
				level: clamp(
					Math.floor(gameState.player.averagePokemonLevel * 0.9) -
						randomInt(0, 5),
					1,
					50,
				),
...
export function spawnChampionTeam(
	...
	for (let i = 0; i < numberToSpawn; i++) {
		...
		team.push(
			new PokemonOnBoard({
				...
				owner: OWNER_TRAINER,
				level: clamp(
					(gameState.player.badges.length + 1) * 5 + (i % 5),
					1,
					100,
				),
				x,
...
export function spawnTrainerTeam(pokemons: PokemonEntry[]): PokemonOnBoard[] {
	...
	for (let i = 0; i < numberToSpawn; i++) {
		...
		do {
			...
			level = clamp(
				gameState.player.averagePokemonLevel + randomInt(-4, +0),
				2,
				50,
			);
			x = randomInt(0, 6);
...
export function spawnSafariTeam(): PokemonOnBoard[] {
	...
	const selection = Array.from({ length: NUMBER_TO_SPAWN }).map(() =>
		ponderatedRandomIn(POKEMONS, (p) => p.wildEncounterChance),
	);
...
export function spawnPensionTeam(): PokemonOnBoard[] {
	...
	const selection = pickNRandomIn(
		getNonLegendaryPokemonsOfType(TYPE_NORMAL),
		NUMBER_TO_SPAWN,
	);
