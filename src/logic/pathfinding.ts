...
export function distanceBetweenPokemon(
	pkm1: PokemonOnBoard,
	pkm2: PokemonOnBoard,
): number {
...
export function findClosestReachableTarget(
	pkm: PokemonOnBoard,
	targetsToExclude: PokemonOnBoard[] = [],
): PokemonOnBoard | null {
	let targets = pkm.opponents.filter((p) => !targetsToExclude.includes(p));
	if (pkm.hasAlteration(AlterationType.CONFUSION))
		targets = pkm.team.filter((p) => p !== pkm);
	targets = targets.filter((candidate) => !candidate.untargettable);

	if (targets.length === 0) return null;
	return targets.reduce((closest, enemy) => {
		return distanceBetweenPokemon(pkm, closest) <
			distanceBetweenPokemon(pkm, enemy)
			? closest
			: enemy;
	});
}

export function findPathToXY(
	pkm: PokemonOnBoard,
...
export function findAnotherTarget(
	...
	if (path.length === 0)
		return findAnotherTarget(pokemon, [...targetsToExclude, anotherTarget]); // cant reach neither
	else return anotherTarget;
}

export function tryToGetCloserToTarget(
	pokemon: PokemonOnBoard,
	target: PokemonOnBoard,
): PF.path {
	...
	let distance = 1,
		...
		y;
	do {
		const delta = getDeltaFromDirection(
			directionsInOrderOfPriority[currentDirection],
		);
