...
import { GEMMES, type Item } from "../data/items";
...
import { PokemonOnBoard, makePokemonSprite } from "../objects/pokemon";
import {
	displayPokemonCaptureInfo,
	hidePokemonCaptureInfo,
} from "../objects/pokemonCaptureBox";
import {
	displayPokemonInfo,
	getCurrentPokemonInfoDisplayed,
	hidePokemonInfo,
} from "../objects/pokemonInfoBox";
import { hidePokemonReleaseInfo } from "../objects/pokemonReleaseBox";
import type GameScene from "../scenes/GameScene";
import { type RoomBoard, RoomType, type RoomWild } from "../types/destination";
...
import { type Player, getAlliancesState } from "./player";
...
export function drawPokemonsOnBoard(game: GameScene) {
	if (
		[
			...
			RoomType.TUTORIAL,
			RoomType.PENSION,
		].includes(gameState.currentRoom.type)
	) {
...
export function getPokemonOnTile(x: number, y: number) {
	return gameState.allPokemonsOnBoard.find(
		(pokemon) => pokemon.x === x && pokemon.y === y,
	);
}
...
export function launchPokeball(
	player: number,
	pokeballType: string,
	x: number,
	y: number,
...
export function spawnPokemon(pokemon: PokemonOnBoard, game: GameScene) {
	...
	launchPokeball(pokemon.owner, pokemon.pokeball, x, y, game).then(
		(pokeball) => {
...
export function drawGrid() {
	...
	const opacity = [RoomType.SAFARI, RoomType.PENSION].includes(
	...
	for (let x = 0; x < BOARD_WIDTH; x++) {
		for (let y = 0; y < BOARD_HEIGHT; y++) {
			...
			gridTile.on(
				"dropReceived",
				(droppedSprite: Phaser.GameObjects.Sprite) => {
					const droppedType = droppedSprite.getData("type");
					if (droppedType === "pokemon")
						dropPokemonOnBoard(droppedSprite, x, y, game);
					else if (droppedType === "item")
						dropItemOnPokemon(droppedSprite, x, y, game);
				},
			);
		}
	}
...
export function setActiveTile(zone: Phaser.GameObjects.Zone, game: GameScene) {
	...
	const gridActiveTile = game.objects.get(
		"gridActiveTile",
...
export function dropPokemonOnBoard(
	...
	const pokemon: Pokemon = sprite.getData("pokemon");
	if (!pokemon) return;
...
export function drawTeamSizeCounter() {
	if ([RoomType.PENSION, RoomType.SAFARI].includes(gameState.currentRoom.type))
		return;
...
export function refreshWildPokemons(game: GameScene) {
	...
	gameState.board.otherTeam = (
		gameState.currentRoom as RoomWild
	).spawnOtherTeam();
	gameState.board.otherTeamAlliances = getAlliancesState(
		gameState.board.otherTeam,
	);
...
export function dropItemOnPokemon(
	droppedSprite: Phaser.GameObjects.Sprite,
	x: number,
	y: number,
	game: GameScene,
	...
	if (pokemon.item != null) {
		const previousItem = pokemon.item;
		defer(() => {
