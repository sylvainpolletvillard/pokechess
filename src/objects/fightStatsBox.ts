...
export function drawFightStats(){
    ...
    function drawPokemonStats(pokemon: PokemonOnBoard, x: number, y: number, playerSide: boolean){
        if(!pokemon) return;
        let px = playerSide ? x : x+220-64
        let py = y - (pokemon.entry.portraitCropY ?? 16) + 1

        const portraitBackground = scene.add.graphics().setDepth(Z.MENU_LAYOUT)
        portraitBackground.fillStyle(0xE8E1D1).lineStyle(1, 0x4E4B46).fillRect(px,y,64,22).strokeRect(px,y,64,22)
        ...
        let rx = playerSide ? x + 64 : x + 220 - 65
        ...
        let w1 = maxWidth*damageDone/maxDamage;
        bars.fillStyle(0xff0000).lineStyle(1, 0x4E4B46).fillRect(playerSide ? rx : rx-w1+1, y+6, w1, 10).strokeRect(playerSide ? rx : rx-w1+1, y+6, w1, 10)
        let text1 = addText(playerSide ? rx+w1+2 : rx-w1-2, y+11, damageDone.toFixed(0), numberStyle).setOrigin(playerSide ? 0 : 1, 0.5).setDepth(Z.MENU_TOOLTIPS)
        box?.add(text1)
