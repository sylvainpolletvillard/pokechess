...
export function makeItemSprite(item: Item) {
	const sprite = gameState
		.activeScene!.add.sprite(0, 0, "items", ITEMS_SPRITES_INDEX.indexOf(item))
		.setOrigin(0.5, 0.5);
...
function selectInList(index: number, shouldMoveCursor = false) {
	...
	if (shouldMoveCursor) {
		scene.sprites
			.get("cursor")
			?.setPosition(298, 20 + list.pageSelectedIndex * 20);
	}
}
...
function drawList() {
	...
	list.page.forEach((entry, i) => {
		const bg = scene.add
			.rectangle(64, 15 + i * 20, 118, 20, i % 2 ? 0xffeedd : 0xeeddcc)
	...
	const arrowUp = scene.add
		.sprite(64, 0, "gui", 16)
		.setScale(0.5)
		.setAlpha(list.pageStartIndex === 0 ? 0 : 1)
	...
	const bgArrowDown = scene.add
		.rectangle(64, 130, 90, 6, 0x000000)
		.setDepth(Z.MENU_LAYOUT)
		.setAlpha(0);
	...
	const arrowDown = scene.add
		.sprite(64, 130, "gui", 16)
		.setScale(0.5)
		.setAlpha(
			list.pageStartIndex + list.numberPerPage >= list.items.length ? 0 : 1,
		)
		.setRotation(Phaser.Math.DegToRad(+90));
