...
export class PlayerCharacter extends Character {
	...
	constructor() {
		...
		this.interactionSprite = gameState.activeScene!.make.sprite({ x:0, y: 0, key: 'interactions' });
	...
	get canMove(){
		return !gameState.activeDialog && !gameState.activeMenu && !this.isForceMoving;
	}
	...
	checkInFrontOfPlayer() {
		...
		this.interactionSprite.setVisible(this.canInteract && !!(pnjInFront || objectInFront))

		if (pnjInFront) this.interactionSprite.anims.play("interaction_talk", true)
		else if (objectInFront) this.interactionSprite.anims.play("interaction_loot", true)
	}

	updateControls() {
		const isAlignedOnGrid = isBetween((this.sprite.body.x + TILE_SIZE/2) % TILE_SIZE, TILE_SIZE/2-1, TILE_SIZE/2+1)
		                     && isBetween((this.sprite.body.y + TILE_SIZE/2) % TILE_SIZE, TILE_SIZE/2-1, TILE_SIZE/2+1)

		const { moveVector } = getMovementVector(this.sprite.scene)
		const direction = getDirectionFromVector(moveVector)

		if(direction === null && this.isMoving && !this.isForceMoving && isAlignedOnGrid){ // not touching controls, stop asap
			this.stopMoving();
		}
		else if(direction != null && direction !== this.walkingDirection && this.canMove && isAlignedOnGrid){
			this.move(direction)
		}
	...
	goToDoor(doorName: string){
		...
		this.sprite.body.reset(door.x*MAP_SCALING + TILE_SIZE/2, door.y * MAP_SCALING)
		this.alignOnGrid()
		const outDirection = Direction[door.properties["outDirection"] as Direction] ?? Direction.UP
