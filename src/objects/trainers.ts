...
export function drawTrainers(game: GameScene) {
	...
	if (
		[RoomType.ARENA, RoomType.TUTORIAL, RoomType.PENSION].includes(
			game.state.currentRoom.type,
		)
	) {
		const arena = game.state.currentRoom as RoomArena;
		const trainer = game.add
			.sprite(game.scale.width + 40, 32, "trainer")
			.setDepth(Z.TRAINER)
