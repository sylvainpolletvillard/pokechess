...
import {addInteractiveElem, InteractiveElem, removeInteractiveElem, updateCursorHover} from "./cursor";
...
const list = {
    ...
    get filtered(){
        return this.allPokemons
            .filter(pkm => currentTypeFilter === null || pkm.types.includes(currentTypeFilter))
    },
    get page(){
...
let typeSelectedIndex: number = 0;
            ...
            if(cursorZone === CursorZone.LIST && list.selectedIndex != null && pokedexContainer){
                if(moveVector.y > 0){
                    ...
                    if(list.selectedIndex === 0){
                        ...
                        selectInList(list.selectedIndex-1)
                        game.sprites.get("cursor")?.setPosition(298, 60 + list.selectedIndex*20)
                    }
                }
...
function drawPokedex(){
    ...
    pokedexContainer.add(
        game.add.line(140,28, 0, 0, 272,0, 0x000000)
            .setLineWidth(0.5)
            .setDepth(Z.MENU_LAYOUT)
    )

    pokedexContainer.add(
        game.add.line(179,132, 0, 0,0,210, 0x000000)
            .setLineWidth(0.5)
            .setDepth(Z.MENU_LAYOUT)
    )
    ...
    sortedTypes.forEach((type: PokemonType) => {
        ...
        if(currentTypeFilter === type){
            pokedexContainer?.add(game.add.graphics()
                .lineStyle(4, 0x000000)
                .strokeCircle(20  + i*16, 16, 8)
            )
        }
        const typeSprite = game.add.sprite(20 + i*16, 16, "icons16x16", type.frameIndex)
...
function drawList(){
    ...
    list.page.forEach((pokemon, i) => {
        let pokemonNum = POKEMONS.indexOf(pokemon) + 1
        const bg = scene.add.rectangle(227, 42+i*20, 94, 20, i%2 ? 0xffeedd : 0xeeddcc)
            .setDepth(Z.MENU_LAYOUT)
        addInteractiveElem(bg);
        interactiveElems.push(bg);
        bg.on("click", () => selectInList(list.filtered.indexOf(pokemon), false))
        const name = addText(188, 35 + i*20, `${pokemonNum}. ${pokemon.name.toUpperCase()}`)!
    ...
    const bgArrowUp = scene.add.rectangle(225, 32, 90, 6, 0x000000)
    ...
    const arrowUp = scene.add.sprite(228, 33, "gui", 16)
        .setScale(0.5).setAlpha(list.pageStartIndex === 0 ? 0 : 0.5)
        .setRotation(Phaser.Math.DegToRad(-90))

    pokedexContainer?.add(bgArrowUp)
    pokedexContainer?.add(arrowUp)

    const bgArrowDown = scene.add.rectangle(225, 234, 90, 6, 0x000000)
        .setDepth(Z.MENU_LAYOUT).setAlpha(0)
    ...
    const arrowDown = scene.add.sprite(228, 233, "gui", 16)
        .setScale(0.5).setAlpha(list.pageStartIndex + list.numberPerPage >= list.filtered.length ? 0 : 0.5)
        .setRotation(Phaser.Math.DegToRad(+90))
...
function selectInList(index: number, shouldMoveCursor = true){
    ...
    drawPokedex()
    if(shouldMoveCursor){
        scene.sprites.get("cursor")?.setPosition(298, 63 + list.pageSelectedIndex*20)
    }
}
...
function drawPokemonInfo(pokemon: PokemonEntry){
    ...
    const pokemonNameText = addText(cx + 36, cy - 18,`${pokemon.name}`, { fontSize: "14px" });
    pokedexContainer.add(pokemonNameText)

    cx += 44
    for(let i=0; i< pokemon.types.length; i++) {
        const typeSprite = game.add.sprite(cx + i*20, cy+10, "icons16x16", pokemon.types[i].frameIndex)
        pokedexContainer.add(typeSprite)
    }
    ...
    pokedexContainer.add(addText(cx+8, cy, "BASE STATS", { fontSize: "10px", color: "white" }))
    cy += 3
    ...
    bars.fillStyle(0x000000).fillRoundedRect(cx+4, cy+14, BAR_WIDTH+2, 6, 4)
    ...
    bars.fillStyle(0x000000).fillRoundedRect(cx+4, cy+34, BAR_WIDTH+2, 6, 4)
    ...
    pokedexContainer.add(addText(cx+2, cy-2, "CAPACITES", { fontSize: "10px", color: "white" }))

    const attackTypeSprite = game.add.sprite(cx+10, cy+20, "icons16x16", pokemon.baseSkill.type.frameIndex)
    ...
    for(let member of family){
        ...
        if(member.evolution){
            ...
            pokedexContainer.add(addText(cx+20, cy+4, `Lv ${member.evolutionLevel??'?'}`, {
                align: "center",
                fontSize: "8px"
            }))
        }
