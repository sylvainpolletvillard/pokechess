...
export class Character {
	...
	constructor(position: {x:number, y:number}, name: string, startState = CHARACTER_STATE.DOWN, interactionDistance = 1) {
		...
		this.sprite = scene.physics.add.sprite(position.x * TILE_SIZE + TILE_SIZE/2, position.y * TILE_SIZE, "characters")
	...
	addAnimations() {
		...
		declareAnims(this.sprite.anims, "characters", [
			...
			[this.name+"_walk_up", [di + 5, di+1, di+6, di+1], WALK_ANIM_SPEED],
			[this.name+"_walk_side", [di + 7, di+2,di+8 , di+2], WALK_ANIM_SPEED]
		])
	}
	...
	moveBack(nbSteps: number){
		...
		switch(this.direction){
			case Direction.LEFT: backwards = Direction.RIGHT; break;
			case Direction.RIGHT: backwards = Direction.LEFT; break;
			case Direction.UP: backwards = Direction.DOWN; break;
			case Direction.DOWN: backwards = Direction.UP; break;
		}
