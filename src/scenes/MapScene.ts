...
} from "../objects/destinationPanel";
import { clickEntry } from "../objects/menu";
import { drawTourCounter } from "../objects/tourCounter";
import type { Destination, Intersection, Path } from "../types/destination";
...
export default class MapScene extends MyScene {
		...
		if (this.isMoving) this.updatePlayerPosition();
			...
			if (moveVector.length() > 1) {
				...
				if (dir != null && this.directions[dir] != null) {
					const destinations =
						this.intersectionReached?.nextDestinations ??
	...
	changeOrigin(newOrigin: Destination) {
		...
		this.player?.setPosition(
			newOrigin.coordinates[0],
			newOrigin.coordinates[1],
		);
	...
	debugIntersections() {
		...
		this.originZone &&
			debugG.fillRect(
				this.originZone.x - 8,
				this.originZone.y - 8,
				this.originZone.width,
				this.originZone.height,
			);
		INTERSECTIONS.forEach((int) => {
			debugG
				.fillCircle(int.coordinates[0], int.coordinates[1], 8)
				.setDepth(9999);
		});
	}
	...
	updatePlayerPosition() {
		...
		if (
			...
			wait(200).then(() => {
				...
				this.player?.setPosition(
					intersectionReached.coordinates[0],
					intersectionReached.coordinates[1],
				);
				this.isMoving = false;
			});
			const onReach =
				intersectionReached.onReach || (() => Promise.resolve(true));
			return onReach().then((canStay: boolean) => {
				if (!canStay) {
					this.walkPath(
						intersectionReached.nextDestinations[this.origin.ref],
						intersectionReached.coordinates,
					);
				} else {
					wait(250).then(() => {
		...
		const destinationReached = isOnOrigin
			? this.origin
			: (this.availableDestinations
		...
		if (this.destinationReached !== destinationReached) {
			if (destinationReached == null && this.destinationReached != null) {
				...
				if (
					!this.destinationSelected ||
					this.destinationSelected === destinationReached
				) {
					wait(350).then(() => {
	...
	drawMapObjects() {
		...
		const boat = this.add
			.sprite(boatX * 16 - 8, boatY * 16 - 8, "map")
			.play("boat");
	...
	updateDestinations() {
		...
		const undeclared = Object.keys(this.origin.nextDestinations).find(
			(ref) => !(ref in DESTINATIONS),
		);
		if (undeclared)
			return console.error(`DESTINATION NOT DECLARED: ${undeclared}`);
		...
		for (const destination of this.availableDestinations) {
			...
			const destinationHighlight = this.add
				.sprite(x, y, "map")
				.play("highlight")
	...
	registerDestination(
		...
		marker: Phaser.GameObjects.Sprite | Phaser.GameObjects.Zone,
	) {
		...
		marker.on("click", () => {
			...
			const currentPos =
				this.destinationReached || this.intersectionReached || this.origin;
	...
	updateDirections(paths: { [ref: string]: Path }) {
		...
		Object.entries(paths).forEach(([ref, path]) => {
			if (
				...
				return; // can only go back
			const dir = getDirectionFromDelta(...path[0]);
			const destination = DESTINATIONS[ref];
			if (dir != null && (!destination.locked || !destination.locked())) {
				this.directions[dir] = ref;
			}
		});
		Object.entries(this.directions).forEach(([dir, dest]) => {
			...
			const dirSprite = this.add
				.sprite(this.player.x + dx * 12, this.player.y + dy * 12, "map", 20)
				.play("map_direction_arrow");
			dirSprite
				.setDepth(Z.MENU_OBJECTS)
				.setRotation(getRotationAngle(dir as Direction));
	...
	walkPath(path: Path, from: [number, number]) {
		...
		path
			.reduce(async (previousStep: Promise<any>, step) => {
				...
				if (
					this.intersectionReached &&
					!arrayEquals(this.intersectionReached.coordinates, from)
				)
